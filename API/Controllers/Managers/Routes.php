 ...
 */
class RoutesManager extends Controller
{
    // * Propiedades 
    ...
    private static function setRouteWithUrl($url)
    {
        ...
        $route = require(dirname(__DIR__, 2) . '/Routes/Root.php');
    ...
    public static function executeMethodWithUrl($url)
    {
        ...
        try {
            call_user_func(self::$routeEnd->getMethodCallable());
            self::getStatus(self::$routeEnd->getStatusCallable());
        } catch (Error $error) {
            self::$status->setStatusWithMessage(
                500,
        ...
        // Valida que su resultado sea 200 ok
        return (self::$status->getStatus() == 200);
    }
    ...
    {
        try {
            self::setResult(call_user_func(self::$routeEnd->getResultCallable()));
        } catch (Error $error) {
            self::$status->setStatusWithMessage(
                500,
